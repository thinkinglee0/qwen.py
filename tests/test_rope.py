...
def test_inv_freq_values():
    rope = BaseRoPE(dim=8, max_seq_len=16, base=10000.0, compile_rope=False)
...
def test_cache_shapes():
    rope = BaseRoPE(dim=8, max_seq_len=16, compile_rope=False)
...
def test_relative_position_invariance():
    ...
    rope = BaseRoPE(dim=64, max_seq_len=256, compile_rope=False)
