...
from rope import init_rope, BaseRoPE
...
class QwenModel(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, input_ids) -> CausalLMOutputWithPast:
        hidden_states = self.embed_tokens(input_ids)    # [bsz, seq_len, hidden_size]
        bsz, q_len, _ = hidden_states.size()

        causal_mask = make_causal_mask(q_len, q_len, hidden_states.device, hidden_states.dtype)
        ...
        for layer in range(self.config.num_hidden_layers):
            ...
            past_len = 0  # replace with the real past_len after M2 adding KV cache
        ...
        logits = hidden_states @ self.weights['lm_head.weight'].transpose(-2, -1)
