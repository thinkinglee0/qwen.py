...
```bash
which python                          # /venv/main/bin/python -- venv guard
python -m venv /opt/vllm-venv
/opt/vllm-venv/bin/pip install -U pip
/opt/vllm-venv/bin/pip install vllm
```

约 10–15 GB(vLLM 自带整套 torch + CUDA runtime)。

**双向验证隔离** —— 两条都要跑,只查一边看不出污染:

```bash
/opt/vllm-venv/bin/python -c "import torch, vllm; print('vllm venv', torch.__version__, vllm.__version__)"
...
用 conda 也行(`conda create -n vllm python=3.12 && conda run -n vllm pip install vllm`);**隔离是硬要求,用什么工具做隔离是次要的**。
